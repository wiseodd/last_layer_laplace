...
val_loader, test_loader = dl.CIFAR100(train=False, val_size=2000)
...
ood_loader = dl.UniformNoise('CIFAR100', delta=delta, size=2000)


def load_model():
    if args.type == 'OE':
        model = resnet_oe.ResNet18(num_classes=100, dataset='CIFAR100').gpu()
    else:
        model = resnet.ResNet18_100().cuda()

    model.load_state_dict(torch.load(f'./pretrained_models/CIFAR100_{args.type}.pt'))
    model.eval()
...
mmc = conf_farway.mean()
auroc = get_auroc(py_in, py_out)
save_res_ood(tab_ood['CIFAR100 - Adversarial'], mmc, auroc)
...
mmc = conf_farway.mean()
auroc = get_auroc(py_in, py_out)
save_res_ood(tab_ood['CIFAR100 - FarAwayAdv'], mmc, auroc)
print(f'[Out-FarAwayAdv, MAP] MMC: {mmc:.3f}; AUROC: {auroc:.3f}')

print()

if args.generate_histograms:
    plot_histogram([conf_in, conf_svhn, conf_farway], ['In - CIFAR100', 'Out - SVHN', 'Out - FarAway'], 'hist_confs_cifar100_map')
...
if args.generate_histograms:
    plot_histogram([conf_in, conf_svhn, conf_farway], ['In - CIFAR100', 'Out - SVHN', 'Out - FarAway'], 'hist_confs_cifar100_temp')
...
if args.compute_hessian:
    ...
    # interval = torch.logspace(-4, 2, 100).cuda()
    # var0 = llla.gridsearch_var0(model, hessians, val_loader, ood_loader, interval, lam=0.25)

    if args.type == 'plain':
        var0 = torch.tensor(1).float().cuda()
    elif args.type == 'ACET':
        var0 = torch.tensor(0.2000).float().cuda()
    else:
        var0 = torch.tensor(2.0092).float().cuda()

    print(var0)

    M_W, M_b, U, V, B = llla.estimate_variance(var0, hessians)
    np.save(f'./pretrained_models/CIFAR100_{args.type}_llla.npy', [M_W, M_b, U, V, B])
else:
    time_inf = 0
    M_W, M_b, U, V, B = list(np.load(f'./pretrained_models/CIFAR100_{args.type}_llla.npy', allow_pickle=True))
...
if args.generate_histograms:
    plot_histogram([conf_in, conf_svhn, conf_farway], ['In - CIFAR100', 'Out - SVHN', 'Out - FarAway'], 'hist_confs_cifar100_llla')
...
if args.generate_figures:
    ...
    def plot_conf(confs, name='figs/conf_map_cifar10', ylabel='Conf. (MAP)'):
        ...
        plt.fill_between(deltas, np.maximum(1/100, mean-3*std), np.minimum(1, mean+3*std), alpha=0.15)
        plt.plot(deltas, mean, lw=3)

        plt.axhline(1/100, lw=3, ls='--', c='k')
    ...
    plot_conf(confs_map, 'figs/conf_map_cifar100_', 'Conf. (MAP)')
    plot_conf(confs_temp, 'figs/conf_temp_cifar100_', 'Conf. (Temp.)')
    plot_conf(confs_laplace, 'figs/conf_laplace_cifar100_', 'Conf. (LLLA)')
...
if args.compute_hessian:
    ...
    # var0 = model_dla.gridsearch_var0(val_loader, ood_loader, interval, n_classes=100, lam=0.25)

    if args.type == 'plain':
        var0 = torch.tensor(1.2130e-05).float().cuda()
    elif args.type == 'ACET':
        var0 = torch.tensor(8.9021e-06).float().cuda()
    else:
        var0 = torch.tensor(3.2745e-05).float().cuda()

    print(var0)

    model_dla.estimate_variance(var0)
    torch.save(model_dla.state_dict(), f'./pretrained_models/CIFAR100_{args.type}_dla.pt')
else:
    time_inf = 0
    model_dla.load_state_dict(torch.load(f'./pretrained_models/CIFAR100_{args.type}_dla.pt'))
    model_dla.eval()
...
if args.generate_histograms:
    plot_histogram([conf_in, conf_svhn, conf_farway], ['In - CIFAR100', 'Out - SVHN', 'Out - FarAway'], 'hist_confs_cifar100_dla')
...
if args.compute_hessian:
    ...
    if args.type == 'plain':
        var0 = torch.tensor(2.7280e-05).float().cuda()
    elif args.type == 'ACET':
        var0 = torch.tensor(2.2570e-05).float().cuda()
    else:
        var0 = torch.tensor(5.2140e-05).float().cuda()

    print(var0)

    model_kfla.estimate_variance(var0)
    torch.save(model_kfla.state_dict(), f'./pretrained_models/CIFAR100_{args.type}_kfla.pt')
else:
    time_inf = 0
    model_kfla.load_state_dict(torch.load(f'./pretrained_models/CIFAR100_{args.type}_kfla.pt'))
    model_kfla.eval()
...
if args.generate_histograms:
    plot_histogram([conf_in, conf_svhn, conf_farway], ['In - CIFAR100', 'Out - SVHN', 'Out - FarAway'], 'hist_confs_cifar100_kfla')
...
with open(f'results/tab_ood_cifar100_{args.type}_{args.randseed}.pkl', 'wb') as f:
    pickle.dump(tab_ood, f)

with open(f'results/tab_cal_cifar100_{args.type}_{args.randseed}.pkl', 'wb') as f:
