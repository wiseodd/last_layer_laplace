...
model_dkl, likelihood = dkl.get_dkl_model(dataset='CIFAR100')

model_dkl.cuda()
likelihood.cuda()

state = torch.load(f'./pretrained_models/CIFAR100_dkl.pt')
...
if args.generate_histograms:
    plot_histogram([conf_in, conf_emnist, conf_farway], ['In - CIFAR100', 'Out - SVHN', 'Out - FarAway'], 'hist_confs_cifar10_dkl')

with open(f'results/tab_ood_cifar100_dkl_{args.randseed}.pkl', 'wb') as f:
    pickle.dump(tab_ood, f)

with open(f'results/tab_cal_cifar100_dkl_{args.randseed}.pkl', 'wb') as f:
